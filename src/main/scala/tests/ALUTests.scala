...
class ALUTests(c: ALUTop) extends Tester(c) {
  for (i <- 0 until 25) {
    ...
    println("*** ADD ***")
    poke(c.io.opcode, Opcode.ARI_RTYPE.litValue())
    poke(c.io.funct, Funct3.ADD_SUB.litValue())
    ...
    println("*** SRL ***")
    poke(c.io.opcode, Opcode.ARI_RTYPE.litValue())
    poke(c.io.funct, Funct3.SRL_SRA.litValue())
    ...
    println("*** ADDI ***")
    poke(c.io.opcode, Opcode.ARI_ITYPE.litValue())
    poke(c.io.funct, Funct3.ADD_SUB.litValue())
    poke(c.io.add_rshift_type, AddRshiftType.ADD.litValue())
    expect(c.io.out, sum)
