...
class Datapath extends Module with CoreParams {
  ...
  val rs1 = Mux(io.ctrl.wb_sel === WB_ALU && rs1hazard, ew_alu, regFile.io.rdata1) 
  val rs2 = Mux(io.ctrl.wb_sel === WB_ALU && rs2hazard, ew_alu, regFile.io.rdata2)
  ...
  io.dcache.req.bits.mask := MuxLookup(io.ctrl.st_type, UInt("b0000"), Seq(
       ST_SW -> UInt("b1111"),
       ST_SH -> (UInt("b11") << alu.io.sum(1,0)),
       ST_SB -> (UInt("b1")  << alu.io.sum(1,0)) ))
  
  // CSR input
  val csr_in = Mux(io.ctrl.imm_sel === IMM_Z, immGen.io.out, rs1)
